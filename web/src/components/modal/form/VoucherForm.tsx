...
import { DiscountDefaultType, DiscountFormProps } from "../../../types/DiscountDefaultType.ts";
import { DiscountDefaultValue } from "../../../constants/DiscountDefaultValue.ts";

export default function VoucherForm({ voucher, show, onHide, onReload }: DiscountFormProps) {
    const [validated, setValidated] = useState(false);
    const [formData, setFormData] = useState<DiscountDefaultType>(DiscountDefaultValue);
    ...
    useEffect(() => {
        ...
        if (voucher) {
            setFormData({
                ...
                start_date: voucher.start_date ? new Date(voucher.start_date) : new Date(), // Chuyển thành đối tượng Date
                end_date: voucher.end_date ? new Date(voucher.end_date) : new Date() // Chuyển thành đối tượng Date
            });
            setFromDate(voucher.start_date ? new Date(voucher.start_date) : new Date());
            setToDate(voucher.end_date ? new Date(voucher.end_date) : new Date());
        } else {
            setFormData(DiscountDefaultValue);
        }
    }, [voucher]);
